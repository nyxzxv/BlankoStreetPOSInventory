...
class BSCPOSInventorySupabaseUser extends BaseAuthUser {
  BSCPOSInventorySupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> bSCPOSInventorySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BSCPOSInventorySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
